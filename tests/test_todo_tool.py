...
def test_list_todos_caches(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "GET", "https://api.example.com/todos", 200, [{"id": "1", "title": "Test", "status": "open"}])
...
def test_create_todo_validates(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "POST", "https://api.example.com/todos", 201, {"id": "2", "title": "Hello", "status": "open"})
...
def test_update_todo(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "PUT", "https://api.example.com/todos/2", 200, {"id": "2", "title": "Hi", "status": "done"})
...
def test_delete_todo(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "DELETE", "https://api.example.com/todos/2", 200, {"id": "2", "title": "Hi", "status": "done"})
...
def test_retry_on_500_succeeds(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "GET", "https://api.example.com/todos", 500, {"error": "boom"})
        add_response(
            rsps,
            "GET",
...
def test_400_errors_do_not_retry(todo_tool: TodoServiceTool) -> None:
    with responses.RequestsMock() as rsps:
        add_response(rsps, "POST", "https://api.example.com/todos", 400, {"error": "bad"})
